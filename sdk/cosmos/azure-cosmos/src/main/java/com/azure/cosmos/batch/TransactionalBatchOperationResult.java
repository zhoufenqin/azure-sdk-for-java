...
//ORIGINAL LINE: internal static Result ReadOperationResult(Memory<byte> input, out TransactionalBatchOperationResult batchOperationResult)
	public static Result ReadOperationResult(Memory<Byte> input, tangible.OutObject<TransactionalBatchOperationResult> batchOperationResult)
	{
	...
	private static Result ReadOperationResult(tangible.RefObject<RowReader> reader, tangible.OutObject<TransactionalBatchOperationResult> batchOperationResult)
	{
...
//ORIGINAL LINE: batchOperationResult.ResourceStream = new MemoryStream(buffer: resourceBody, index: 0, count: resourceBody.Length, writable: false, publiclyVisible: true);
//C# TO JAVA CONVERTER TODO TASK: C# to Java Converter cannot determine whether this System.IO.MemoryStream is input or output:
					batchOperationResult.argValue.setResourceStream(new MemoryStream(buffer: resourceBody, index: 0, count: resourceBody.Length, writable: false, publiclyVisible: true));
					break;
...
//C# TO JAVA CONVERTER TODO TASK: The following method call contained an unresolved 'out' keyword - these cannot be converted using the 'OutObject' helper class unless the method is within the code being modified:
					...
					batchOperationResult.argValue.setRequestCharge(Double.isNaN(requestCharge) ? Double.NaN : Math.round(requestCharge * Math.pow(10, 2)) / Math.pow(10, 2));
					break;
...
//ORIGINAL LINE: ResponseMessage responseMessage = new ResponseMessage(statusCode:this.StatusCode, requestMessage: null, errorMessage: null, error: null, headers: headers, diagnostics: this.DiagnosticsContext ?? new CosmosDiagnosticsContext())
		ResponseMessage responseMessage = new ResponseMessage(statusCode:this.getStatusCode(), requestMessage: null, errorMessage: null, error: null, headers: headers, diagnostics: this.getDiagnosticsContext() != null ? this.getDiagnosticsContext() : new CosmosDiagnosticsContext())
		{
